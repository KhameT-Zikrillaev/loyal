...
async function fletchcard() {
  try {
    const response = await axiosCustom.get("faqs/");
...
const addProduct = async () => {
  ...
  try {
    await axiosCustom.post(`faqs/`, formData);
    toast.success("Added new faqs successfully", {
      autoClose: 2000,
      theme: "colored"
    });
    document.getElementById("myFormFaqs").reset();
    addmodal(false);
    fletchcard();
  } catch (error) {
...
const editChangeProduct = async ()=>{
  ...
  try {
    await axiosCustom.put(`faqs/${idtovar.value}`,formData, {
    });
    toast.success("Faqs is edited successfully", {
      autoClose: 2000,
      theme: "colored"
    });
        fletchcard();
...
const deleteProduct = async ()=>{
  try {
    await axiosCustom.delete(`faqs/${idtovar.value}`)
...
<template>
  ...
  <!-- Modal ADD -->
  <a-modal v-model:open="modaladd" title="Add new Faq" @ok="addProduct">
    <a-form id="myFormFaqs" layout="vertical" style="margin-top: 20px;">
  ...
  <!-- Modal EDIT-->
  <a-modal v-model:open="modaledit" title="Edit Faq" @ok="editChangeProduct">
...
<a-form layout="vertical" style="margin-top: 20px;">
  ...
  <!-- Modal Delete-->
  <a-modal v-model:open="modaldelete" title="Delete Faq" @ok="deleteProduct">
