...
async function fletchcard() {
  try {
    const response = await axiosCustom.get("news/");
...
const addProduct = async () => {
  ...
  try {
    await axiosCustom.post(`news/`, formData);
    toast.success("Added new faqs successfully", {
      autoClose: 2000,
      theme: "colored"
    });
    document.getElementById("myFormNews").reset();
    addmodal(false);
    fletchcard();
  } catch (error) {
...
<template>
  ...
  <!-- Modal ADD -->
  <a-modal v-model:open="modaladd" title="Add new news" @ok="addProduct">
    <a-form id="myFormNews" layout="vertical" style="margin-top: 20px;">
